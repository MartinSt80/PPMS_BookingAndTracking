...
class LoggedSessions:
        ...
        for log_file_path in log_file_list:
            ...
            for line in used_time:
                ...
                if start_datetime != last_datetime:
                    ppms_session = Session(start_datetime, last_datetime, facility_id, system_id, system_name,
                                           user_login)
    ...
    def log_used_time(self):
        ...
        for used_session in self.session_list:
            ...
            # Try to book the session, succeeds if there is no conflicting session
            # (e.g. User has not used PPMS to book the session)
            ...
            try:
                ...
                print(
                    f'Session {self.booking_id} booked for {self.user_login} from {self.start_time} to {self.stop_time}')


class CachedBookedSessions:

    def __int__(self, PPMS_facility_id: str):

        self.PPMS_facility_id = PPMS_facility_id
    ...
    def getSessionsofDay(self, day: datetime.date) -> list:
        try:
            ...
            day_index = len(self.cached_day_list)
            self.cached_day_list.append(day)
            self.cached_session_list.append(self.ppms_call.getBookedSessionsPeriod(day, day, self.PPMS_facility_id))
        finally:
            return self.cached_session_list[day_index]
...
if __name__ == "__main__":
    ...
    argument_parser.add_argument("log_file_path", type=str,
                                 help='Log file or directory tracking the used time on the instrument')
    ...
    cached_sessions = CachedBookedSessions()
