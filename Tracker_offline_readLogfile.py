...
def transfer_logs_to_ppms():
    ...
    class Session:
        ...
        def __init__(self, start, stop, fac_id, sys_id, user):
            ...
            self.session_info = self.__create_session_info()
            self.booking_id = None

        def __create_session_info(self):
    ...
    def create_session_overview_for_stratocore():

        session_file_name = 'booked_sessions.csv'

        if log_path.is_dir():
            session_file_path = log_path / session_file_name
        if log_path.is_file():
            session_file_path = log_path.with_name(session_file_name)
    ...
    if log_path.is_dir():
        log_file_list = [file_path for file_path in log_path.glob('*.csv')]
        log_file_list.sort()
    if log_path.is_file():
